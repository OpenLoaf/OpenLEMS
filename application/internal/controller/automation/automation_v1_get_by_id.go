...
func (c *Controller) GetAutomationById(ctx context.Context, req *v1.GetAutomationByIdReq) (res *v1.GetAutomationByIdRes, err error) {
	c_log.Infof(ctx, "获取自动化任务详情 - ID: %d", req.Id)
	...
	if err != nil {
		c_log.Errorf(ctx, "获取自动化任务失败 - ID: %d, 错误: %+v", req.Id, err)
		return nil, gerror.WrapCode(gcode.CodeInternalError, err, "获取自动化任务失败")
	}

	// 检查记录是否存在
	if automation == nil {
		c_log.Warningf(ctx, "自动化任务不存在 - ID: %d", req.Id)
	...
	if err := automationEntity.UnmarshalValue(automation); err != nil {
		c_log.Errorf(ctx, "转换自动化实体失败 - ID: %d, 错误: %+v", req.Id, err)
	...
	c_log.Infof(ctx, "成功获取自动化任务详情 - ID: %d", req.Id)
	return res, nil
}

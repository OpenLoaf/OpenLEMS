...
func ParseCanbusData(canData []byte, meta *c_base.Meta) (any, error) {
	...
	if int(meta.Addr) >= len(canData) && !(meta.ReadType >= c_base.RBit0 && meta.ReadType <= c_base.RBit15) {
		return nil, gerror.Newf("Address %d out of bounds for data length %d for non-bit read type", meta.Addr, len(canData))
	}
	...
	case c_base.RBcd16:
		...
		if int(meta.Addr)+2 > len(canData) {
			return nil, gerror.Newf("BCD16 read out of bounds. Addr: %d, Data Len: %d", meta.Addr, len(canData))
		}
		...
		if meta.Endianness == c_base.EBigEndian {
			...
			processedBytes := meta.Endianness.FillUpSize(dataBytes, 2)
			finalBCDVal = float64(
...
func parseBitValue(canData []byte, meta *c_base.Meta) (any, error) {
	...
	if endByteIndex > len(canData) {
		return nil, gerror.Newf("Bit read data range out of bounds. Start byte: %d, End byte (exclusive): %d, Data Len: %d", startByteIndex, endByteIndex, len(canData))
	}
	...
	reorderedBytes := meta.Endianness.FillUpSize(relevantBytes, len(relevantBytes))
	...
	if msbStartIndex < 0 || msbEndIndex > totalBitsInReorderedBytes {
		return nil, gerror.Newf("Bit extraction within reordered bits out of bounds. MSB start: %d, MSB end: %d, Total bits: %d", msbStartIndex, msbEndIndex, totalBitsInReorderedBytes)
	}
	...
	// 3. 将字节转换为最终的无符号整型。
	filledBits := meta.Endianness.FillUpSizeBit(targetBits, actualBitLength)
...
func convertBitsToUint(dataBytes []byte, bitLength int, endianness c_base.ECharSequence) (any, error) {
	...
	default:
		return nil, gerror.Newf("Bit length %d exceeds max supported 64-bit integer", bitLength)
	}

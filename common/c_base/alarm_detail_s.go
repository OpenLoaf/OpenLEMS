...
func (s *SAlarmDetail) ToString() string {
	if s.IsTrigger {
		return fmt.Sprintf("++ 触发 ID：%s [%s], 点位: %s(%s),发生时间: %s,数值: %v", s.DeviceId, s.Level.Name(), s.Meta.Name, s.Meta.Cn, s.HappenTime.Format("2006-01-02 15:04:05"), s.Value)
	} else {
		return fmt.Sprintf("-- 解除 ID：%s [%s], 点位: %s(%s),发生时间: %s,数值: %v", s.DeviceId, s.Level.Name(), s.Meta.Name, s.Meta.Cn, s.HappenTime.Format("2006-01-02 15:04:05"), s.Value)
	}
}

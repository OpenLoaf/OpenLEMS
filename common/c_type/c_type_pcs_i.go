...
type IPcsBasic interface {
	...
	GetIGBTTemperature() (*float32, error) // IGBT 温度
}
...
type IPcs interface {
	...
	SetReset() error                                  // 复位
	SetStatus(status c_enum.EEnergyStoreStatus) error // 设置状态(开机、关机、待机、充电、放电、故障、同步中
	SetGridMode(mode c_enum.EGridMode) error          // 设置电网状态
	SetPower(power int32) error                       // 设置有功功率
	SetReactivePower(power int32) error               // 设置无功功率
	SetPowerFactor(factor float32) error              // 设置功率因数

	GetStatus() (*c_enum.EEnergyStoreStatus, error) // 状态
	GetGridMode() (*c_enum.EGridMode, error)        // 电网状态

...
func (s *sAutomationServiceImpl) CreateAutomation(ctx context.Context, name string, startTime, endTime *time.Time, timeRangeType, timeRangeValue *string, triggerRule, executeRule string, executionInterval int) (int, error) {
	...
	if err != nil {
		c_log.Errorf(ctx, "创建自动化规则失败 - 错误: %+v", err)
		return 0, err
	}
	...
	if err != nil {
		c_log.Errorf(ctx, "获取自动化规则ID失败 - 错误: %+v", err)
		return 0, err
	}

	lastId := lastIdValue.Int()

	c_log.Infof(ctx, "成功创建自动化规则，ID: %d", lastId)
	return lastId, nil
}
...
func (s *sAutomationServiceImpl) GetAutomationById(ctx context.Context, id int) (*s_db_model.SAutomationModel, error) {
	...
	if err != nil {
		c_log.Errorf(ctx, "获取自动化规则失败，ID: %d - 错误: %+v", id, err)
		return nil, err
	}
...
func (s *sAutomationServiceImpl) UpdateAutomation(ctx context.Context, id int, data map[string]interface{}) error {
	c_log.Infof(ctx, "开始更新自动化规则，ID: %d, 数据: %+v", id, data)
	...
	c_log.Infof(ctx, "更新字段: %+v", updateFields)
	err = automation.UpdateFields(ctx, updateFields)
	if err != nil {
		c_log.Errorf(ctx, "更新自动化规则失败，ID: %d - 错误: %+v", id, err)
		return err
	}

	c_log.Infof(ctx, "成功更新自动化规则，ID: %d, 最终状态: Enabled=%t", id, automation.Enabled)
	return nil
}
...
func (s *sAutomationServiceImpl) DeleteAutomation(ctx context.Context, id int) error {
	...
	if err != nil {
		c_log.Errorf(ctx, "删除自动化规则失败，ID: %d - 错误: %+v", id, err)
		return err
	}

	c_log.Infof(ctx, "成功删除自动化规则，ID: %d", id)
	return nil
}
...
func (s *sAutomationServiceImpl) GetAllAutomations(ctx context.Context) ([]*s_db_model.SAutomationModel, error) {
	...
	if err != nil {
		c_log.Errorf(ctx, "获取所有自动化规则失败 - 错误: %+v", err)
		return nil, err
	}
...
func (s *sAutomationServiceImpl) GetAutomationsByTimeRangeType(ctx context.Context, timeRangeType string) ([]*s_db_model.SAutomationModel, error) {
	...
	if err != nil {
		c_log.Errorf(ctx, "根据时间范围类型获取自动化规则失败，类型: %s - 错误: %+v", timeRangeType, err)
		return nil, err
	}
	return automations, nil
}
...
func (s *sAutomationServiceImpl) GetEnabledAutomations(ctx context.Context) ([]*s_db_model.SAutomationModel, error) {
	...
	if err != nil {
		c_log.Errorf(ctx, "获取启用的自动化规则失败 - 错误: %+v", err)
		return nil, err
	}
	return automations, nil
}
...
func (s *sAutomationServiceImpl) buildAutomationQuery(ctx context.Context, deviceId string, filters map[string]interface{}) *gdb.Model {
	...
	if deviceId != "" {
		c_log.Infof(ctx, "构建设备ID过滤条件: %s", deviceId)

		// 使用优化的JSON查询条件
		model = s.buildDeviceIdQuery(model, deviceId)
		c_log.Infof(ctx, "应用设备ID过滤条件完成")
	}
...
func (s *sAutomationServiceImpl) GetAutomationsByFilters(ctx context.Context, deviceId string, filters map[string]interface{}) ([]*s_db_model.SAutomationModel, error) {
	...
	if err != nil {
		c_log.Errorf(ctx, "根据过滤条件获取自动化规则失败 - 错误: %+v", err)
		return nil, err
	}

	c_log.Infof(ctx, "成功获取自动化规则，数量: %d", len(automations))
	return automations, nil
}
...
func (s *sAutomationServiceImpl) GetAutomationPage(ctx context.Context, page, pageSize int, deviceId string, filters map[string]interface{}) ([]*s_db_model.SAutomationModel, int, error) {
	c_log.Infof(ctx, "开始分页查询自动化规则 - 页码: %d, 每页数量: %d, 设备ID: %s", page, pageSize, deviceId)
	...
	if err != nil {
		c_log.Errorf(ctx, "获取自动化规则总数失败 - 错误: %+v", err)
		return nil, 0, err
	}

	c_log.Infof(ctx, "查询到自动化规则总数: %d", total)

	// 分页查询
	var automations []*s_db_model.SAutomationModel
	err = model.Page(page, pageSize).Scan(&automations)
	if err != nil {
		c_log.Errorf(ctx, "分页获取自动化规则失败 - 错误: %+v", err)
		return nil, 0, err
	}

	c_log.Infof(ctx, "成功获取自动化规则分页数据 - 返回数量: %d", len(automations))
...
func (s *sAutomationServiceImpl) ClearAllAutomations(ctx context.Context) error {
	...
	if err != nil {
		c_log.Errorf(ctx, "清空所有自动化规则失败 - 错误: %+v", err)
		return err
	}

	c_log.Info(ctx, "成功清空所有自动化规则")
	return nil
}

...
func (d *Device) Create(ctx context.Context) error {
	_, err := g.Model("device").Ctx(ctx).Insert(d)
	return err
}
...
func (d *Device) GetByName(ctx context.Context, name string) error {
	return g.Model("device").Ctx(ctx).Where("name", name).Scan(d)
}

// Update 更新设备记录
func (d *Device) Update(ctx context.Context) error {
	_, err := g.Model("device").Ctx(ctx).Where("id", d.Id).Update(d)
	return err
}

// UpdateFields 更新指定字段
func (d *Device) UpdateFields(ctx context.Context, data g.Map) error {
	_, err := g.Model("device").Ctx(ctx).Where("id", d.Id).Update(data)
	return err
}

// Delete 删除设备记录
func (d *Device) Delete(ctx context.Context) error {
	_, err := g.Model("device").Ctx(ctx).Where("id", d.Id).Delete()
	return err
}
...
func GetAllDevices(ctx context.Context) ([]*Device, error) {
	var devices []*Device
	err := g.Model("device").Ctx(ctx).Scan(&devices)
	return devices, err
}
...
func GetDevicesByCondition(ctx context.Context, condition g.Map) ([]*Device, error) {
	var devices []*Device
	err := g.Model("device").Ctx(ctx).Where(condition).Scan(&devices)
	return devices, err
}
...
func GetDevicesByGid(ctx context.Context, gid uint) ([]*Device, error) {
	var devices []*Device
	err := g.Model("device").Ctx(ctx).Where("gid", gid).Scan(&devices)
	return devices, err
}
...
func GetEnabledDevices(ctx context.Context) ([]*Device, error) {
	var devices []*Device
	err := g.Model("device").Ctx(ctx).Where("enable", true).Scan(&devices)
	return devices, err
}

// Count 获取设备总数
func CountDevices(ctx context.Context) (int, error) {
	count, err := g.Model("device").Ctx(ctx).Count()
	return count, err
}

// CountByCondition 根据条件获取设备数量
func CountDevicesByCondition(ctx context.Context, condition g.Map) (int, error) {
	count, err := g.Model("device").Ctx(ctx).Where(condition).Count()
	return count, err
}
...
func PaginateDevices(ctx context.Context, page, pageSize int) ([]*Device, error) {
	var devices []*Device
	err := g.Model("device").Ctx(ctx).Page(page, pageSize).Scan(&devices)
	return devices, err
}

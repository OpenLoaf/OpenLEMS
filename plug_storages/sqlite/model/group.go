...
func GetAllGroups(ctx context.Context) ([]*Group, error) {
	var groups []*Group
	err := g.Model("group").Ctx(ctx).Scan(&groups)
	return groups, err
}
...
func GetGroupsByCondition(ctx context.Context, condition g.Map) ([]*Group, error) {
	var groups []*Group
	err := g.Model("group").Ctx(ctx).Where(condition).Scan(&groups)
	return groups, err
}
...
func GetGroupsByVersion(ctx context.Context, version string) ([]*Group, error) {
	var groups []*Group
	err := g.Model("group").Ctx(ctx).Where("version", version).Scan(&groups)
	return groups, err
}

// Count 获取分组总数
func CountGroups(ctx context.Context) (int, error) {
	count, err := g.Model("group").Ctx(ctx).Count()
	return count, err
}

// CountByCondition 根据条件获取分组数量
func CountGroupsByCondition(ctx context.Context, condition g.Map) (int, error) {
	count, err := g.Model("group").Ctx(ctx).Where(condition).Count()
	return count, err
}
...
func PaginateGroups(ctx context.Context, page, pageSize int) ([]*Group, error) {
	var groups []*Group
	err := g.Model("group").Ctx(ctx).Page(page, pageSize).Scan(&groups)
	return groups, err
}

// Exists 检查分组是否存在
func (group *Group) Exists(ctx context.Context) (bool, error) {
	count, err := g.Model("group").Ctx(ctx).Where("id", group.Id).Count()
	return count > 0, err
}

// ExistsByName 根据名称检查分组是否存在
func ExistsGroupByName(ctx context.Context, name string) (bool, error) {
	count, err := g.Model("group").Ctx(ctx).Where("name", name).Count()
	return count > 0, err
}
